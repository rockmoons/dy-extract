...
API 01 返回的播放数始终为 0。用同一 apikey 补查一次即可获得真实值：

```bash
curl -s --connect-timeout 10 --max-time 30 "{douyin.base_url}/dy_fetch_multi_video_statistics?aweme_ids={ID}&apikey={douyin.apikey}"
```
